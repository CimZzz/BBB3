...
import com.gogoh5.apps.quanmaomao.library.entities.databeans.parseCashRenderer
...
class GetCashListRequest(
    val uid: String,
    val page: Int,
    val pageSize: Int = 20
): BaseRequest<JSONObject>(Http.Get.DETAIL_CASH) {
    override fun buildRequest(): Request.Builder =
        Request.Builder()
            .url(url)
            .post(RequestBody.create(null, buildApiEncode("withdrawList") {
                it["uid"] = uid
                it["page"] = page
        ...
        if(result.getIntValue("status") == 0) {
            ...
            if(dataArr != null && dataArr.size != 0) {
                dataArr.filterIsInstance(JSONObject::class.java).forEach {
                    list.add(parseCashRenderer(it) ?:return@forEach)
                }
            }
